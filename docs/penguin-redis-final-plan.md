...
## Command Intelligence Edition · v2.0
...
但 profile 与另一完整 URL 不能默默混合；同时提供时应报冲突。`-n` 可以作为已保存 profile 的本次 DB 覆盖。主机、端口或身份发生覆盖时，不自动把原 profile 的秘密信息发往新目标；必须重新绑定凭证或明确确认可信目标。
...
长 JSON 不默认静默省略。普通 CLI 默认输出完整表格；超过交互输出阈值时明确询问“完整输出 / 已保留结果浏览 / 授权落盘 / 取消”。用户显式启用 preview 后才显示折叠，并在表外写清：`preview only; value retained` 或 `value not fully retained`。
...
`Ctrl+R` 搜索当前 connection ID、身份和 DB 的历史。默认保存读命令结构与允许的参数，不保存回复；写 payload、AUTH、token、含凭证 URL 不持久化。key 本身也可能敏感，profile 可禁用历史或将 key 参数也脱敏。
...
| 任意二进制参数 | 引号、空值、NUL、无效 UTF-8 | 安全展示与精确插入分离 |
...
前缀搜索要对 Redis glob 特殊字符做正确转义；不能将用户输入的 `player:[` 无声变成另一个匹配条件。原始 MATCH 模式仍按用户字节执行，不擅自纠正。
...
| 普通 assistance 增量工作集 | 争取不超过 8 MiB | 不含大结果 store；不是预先分配 |
| 已观察 key 名缓存 | 5,000 个或 2 MiB，先到者为准 | 超长名字受单项限制，仍可手输 |
| field/member 名缓存 | 2 MiB 总预算 | 分 target/key LRU，不按每 key 无限叠加 |
| metadata/docs 解析缓存 | 2 MiB 热工作集起点 | 完整离线文档按需解压，重型模块不全加载 |
| scratch 与候选/索引 | 2 MiB 起点 | 包括索引附加内存，不只计算字符串 bytes |
| 大输入智能解析 | 超过 64 KiB 转有限窗口分析 | 命令本身不因此截断 |
| remote discovery 并发 | 每进程自动发现最多 1 个 | 不抢占普通请求与事务通道 |
| 授准自动发现频率 | 每 profile 最多 2 请求/秒 | 手动任务也有自己的上限，不能无限绕过 |
| 单次自动发现请求数 | 最多 2 次 SCAN 类请求 | 找不到即返回部分结果 |
| remote discovery 时限 | 1 秒目标预算 | 网络关闭/清理另行记录 |
...
用户明确选择共享已批准的书签/schema 时，使用版本化本机文件和权限检查。只读目录映射可能被 OS page cache 共享，这是实现优化，不应对每台机器承诺特定 RSS 数字。
...
如果 editor adapter 无法实现可靠焦点/异步取消，允许补丁或替换该 adapter；Command Intelligence、tokenizer、request/outcome 与测试不跟着改成另一套语义。不要通过启动第二个 REPL 进程来绕开设计。
...
| ASSIST-016 | 局部 token 编辑 | sta|tus 接受 status 后不重复后缀。 |
...
| 用途搜索 | 受测离线任务集 top-3 recall >= 95% | 固定人工标注集，另外报告模糊任务和错误建议 |
...
| 普通 assistance 增量 RAM | 8 MiB 起点 | 比较同进程开关提示的增量；包含索引/缓存开销 |
...
| `--output resp` | 类型与字节安全的协议帧输出 | 输出定义的 RESP framing；不保证未捕获情况下逐字节重现原始 wire |
...
`--raw` 在 TTY 上是用户明确请求的未经展示转义路径；文档告知可能包含终端控制内容。默认 pretty 模式必须安全转义。真正 binary-safe 的脚本不要依赖 raw 文本分隔，使用 typed 或 blob 输出。
...
enum ExecutionOutcome {
    NotSent,
    Confirmed,
    ServerErrorObserved,         // 不自动推出没有副作用
    QueuedInTransaction,         // 不等于已应用
    SentWithoutAcknowledgement, // 显式 CLIENT REPLY 模式，效果未确认
    DeniedByPolicy,
    UnknownAfterSend,
    CancelledBeforeSend,
    PartialBatch,
...
解码层覆盖简单/批量字符串、整数、double、big number、boolean、null、array、map、set、verbatim string、error、attribute 和 push；按实际协议支持范围记录版本。[R02]
...
每个请求记录：来源、脱敏命令名、请求/响应字节、发送状态、目标节点、明确发生的重定向、首字节时间、接收完成时间、渲染完成时间和结果保留范围。
...
Execution result 还需要独立的 `EffectsCertainty`：no-command-sent、reply-observed、condition-not-applied、effects-possible、known-partial、unacknowledged-by-mode 等。**收到 server error 不代表所有副作用都没发生。** 例如复合操作、脚本或 EXEC 内部的错误需要保留逐项/部分状态；不把一个 error 字符串统一映射为“安全重试”。

最终模型应把 transport delivery、server reply、application effects 和 local render status 分成字段。UI 显示成功/失败的来源要清楚：渲染失败不能把服务器已执行的写入标成未执行，脚本错误也不能被描述成自动事务回滚。[R11]
...
兼容目标分三类：发布基线中的 Redis 版本；单独受测的 Redis-compatible 实现；受限命令的云/托管部署。协议相似不等于所有命令、ACL 和 Cluster 行为相同。
...
Redis Cluster 的重定向与 key slot 规则是服务器语义的一部分，多 key 命令存在同 slot 限制；只有 DB 0 可用。[R10]
...
Redis ACL 是服务器端的命令、key 和身份访问边界。[R15] 客户端 guardrail 主要防误操作，不能防止具有同一权限的人改用其他客户端；本地确认也不能提升服务器权限。
...
审批绑定 profile ID、endpoint/trust fingerprint、DB、用户身份、命令与参数摘要、key 范围、最大动作数和有效期。任一关键内容变化都使批准失效。
...
Assistance 的 8 MiB 起始目标是新增热工作集目标，不是无条件额外预留；result cache 的 64 MiB 也是最大保留预算，不代表空闲时已全部占用。整进程 budget manager 统一记账，避免模块各自达到上限后突破进程预算。
...
由明确调用的 MCP host 启动子进程，通过 stdin/stdout 通讯；stdout 仅用于协议，日志走 stderr。MCP 规范提供 stdio transport。[R29] 不需要为普通使用另起 HTTP 服务。
...
批准由可信的人类入口或已批准策略完成；agent 不能自行创建自己的 approval。MCP 的输入、返回内容和第三方工具需要按信任边界处理，安全设计以官方建议为基础并落实到本地策略。[R30]
...
插件有 CPU、内存、输出大小和超时预算；崩溃时退回 generic view，不影响已完成请求。主题不携带执行脚本；导入一个主题不能获得访问 credential store 的能力。
...
| REPL | Reedline + 受控 IdeMenu/Completion adapter | 自动下拉/异步更新/按键策略必须固定版本验收 [R28][R47][R48][R49] |
...
| JSON | 保留原文 + token-aware parser/formatter | 避免数值精度、重复 key 和复制内容变化 |
...
| Secret store | `keyring-core` + 明确的平台 store 适配器 | 按平台控制能力，不默认链接所有 provider [R33] |
| 配置 | TOML + schema version + 原子写入 | 人可读、可审阅、可迁移 |
| History | 内嵌 SQLite 或等价受测实现 | 并发/搜索/迁移；无额外服务 |
...
`--help`、`--version`、`--demo`、`--learn`、`--list` 在不需要时不创建 runtime 网络任务，不解锁 credential store。用户取消后，结构化并发 scope 负责回收子任务；绝不依赖“进程退出自然就好”掩盖长会话泄漏。
...
使用 no-network transport spy 测默认输入；获准远端 discovery 在隔离真实 Redis 上测请求与响应边界。无法自动化验证的系统 IME/屏幕阅读器场景进入发布平台人工记录，不能只写“支持 Unicode”代替验收。
...
| E2 · Daily CLI + Assistance | `prc @dev`、Keychain、Hash/JSON table、colour、自动建议、签名、F1/F2、history、one-shot/raw/JSON | 用户主路径含“记不住命令”；关键 ASSIST 与脚本测试通过 |
...
scope = "profile-service-db-identity-policy-epoch"
